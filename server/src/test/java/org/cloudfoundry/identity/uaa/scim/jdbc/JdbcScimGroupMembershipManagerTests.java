...
public class JdbcScimGroupMembershipManagerTests extends JdbcTestBase {
    ...
    public void canUpdateOrAddMembers() {
        ...
        dao.addMember("g1", new ScimGroupMember("m4", ScimGroupMember.Type.USER, ScimGroupMember.GROUP_MEMBER));
        ...
        List<ScimGroupMember> members = dao.updateOrAddMembers("g1", Arrays.asList(g2, m3, m4));

        validateCount(4);
        verify(spy).updateMember("g1", g2);
        verify(spy, times(0)).updateMember("g1", m4);
        verify(spy).addMember("g1", m3);
        verify(spy, times(0)).addMember("g1", m4);
        verify(spy).removeMemberById("g1", "m1");
        assertEquals(3, members.size());
